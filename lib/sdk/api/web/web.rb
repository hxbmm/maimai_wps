...
module KSO_SDK::Web
  ...
  module Internal
    ...
    class MainWindowCenter < PositionCalculator
      ...
      def getGeometry
        ...
        if left == 0
          left = ($kxApp.currentMainWindow.width - width) / 2 + $kxApp.currentMainWindow.geometry.left
        end
        if top == 0
          top = ($kxApp.currentMainWindow.height - height) / 2 + $kxApp.currentMainWindow.geometry.top
        end
    ...
    class EditWindowCenter < PositionCalculator
      ...
      def getGeometry
        ...
        if !$kxApp.currentMainWindow.centralWidget.nil? and 
          ...
          left = (geo.width - width)/2 + geo.left
          top = (geo.height - height)/2 + y
        end
    ...
    class WebImpl < Qt::Object
      ...
      def setDragArea(left, top, width, height)
        self.web_widget.onSetDragArea(Qt::Rect.new(left, top, width, height)) unless self.web_widget.nil?
      end
  ...
  class WebView < KSO_SDK::JsBridge
    ...
    def navigateOnNewWidget(url:, width:500, height:500, bModal:false, closeBtn:false, posType: 'main_window_center')
      navigateNewWidget(:show_modal, url, 0, 0, width, height, posType, false, false)
    end
